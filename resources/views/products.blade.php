        ...
        <div class="px-6">
            <div class="bg-white rounded-lg px-6 py-6 mt-6">
                <div class="relative flex flex-col w-full text-gray-700 bg-white rounded-lg bg-clip-border">
                    ...
                    <!-- create modal-->
                    <x-modal name="create-product" :show="false">
                        <div class="p-6" x-data="{
                            ...
                            <form method="POST" action="{{ route('products.store') }}" enctype="multipart/form-data">
                                ...
                                <div class="flex justify-end">
                                    <button type="button" @click="$dispatch('close-modal', { name: 'create-product' })"
                            ...
                            <tbody>
                                @foreach ($products as $data)
                                    <tr class="hover:bg-slate-50 border-b border-slate-200">
                                        ...
                                        @if ($data->category)
                                            <td class="p-4 py-5">
                                                <p class="text-sm text-slate-500">{{ $data->category->name }}</p>
                                            </td>
                                        @else
                                            <td class="p-4 py-5">
                                        ...
                                        <td>
                                            <div class="flex flex-row space-x-1">
                                                ...
                                                <x-slot name="content">
                                                    <x-dropdown-link
                                                        @click="$dispatch('open-modal', { name: 'update-product-{{ $data->id }}' })">
                                                        {{ __('Edit') }}
                                                    </x-dropdown-link>
                                                    <x-dropdown-link
                                                        @click="$dispatch('open-modal', { name: 'delete-product-{{ $data->id }}' })">
                                                        {{ __('Hapus') }}
                                                    </x-dropdown-link>
                                                </x-slot>
                                            </x-dropdown>
                                                ...
                                                <!-- update modal -->
                                                <x-modal name="update-product-{{ $data->id }}" :show="false">
                                                    <div class="p-6" x-data="{ changed: false, name: '{{ $data->name }}', barcode: '{{ $data->barcode }}', cons_price: '{{ $data->cons_price }}', selling_price: '{{ $data->selling_price }}', expired: '{{ $data->expired }}' }">
                                                        <h2
                                                        ...
                                                        <form method="POST"
                                                                ...
                                                                <select name="categories_id" x-model="categories_id"
                                                                    ...
                                                                    @foreach ($categories as $category)
                                                                        <option value="{{ $category->id }}"
                                                                            @if ($data->categories_id == $category->id) selected @endif>
                                                                            {{ $category->name }}
                                                                        </option>
                                                                    @endforeach
                                                                </select>
                                                            ...
                                                            <div class="flex justify-end">
                                                                <button type="button"
                                                                    @click="$dispatch('close-modal', { name: 'update-product-{{ $data->id }}' })"
                                                                    class="mr-2 px-4 py-2 text-sm bg-gray-200 rounded">
                                                                    Cancel
                                                                </button>
                                                                <button type="submit"
                                                ...
                                                <!-- delete modal -->
                                                <x-modal name="delete-product-{{ $data->id }}" :show="false">
                                                    <div class="p-6" x-data>
                                                        <h2
                                                            class="text-lg font-semibold mb-4 text-gray-800 dark:text-gray-100 text-left">
                                                            Hapus Produk
                                                        ...
                                                        <form method="POST"
                                                            ...
                                                            <div class="flex justify-end">
                                                                <button type="button"
                                                                    class="mr-2 px-4 py-2 text-sm bg-gray-200 rounded"
                                                                    @click="$dispatch('close-modal', { name: 'delete-product-{{ $data->id }}' })">
                                                                    Tidak, Batalkan
                                                                </button>
                                                                <button type="submit"
                                                                    class="px-4 py-2 text-sm bg-sky-900 text-white rounded">
